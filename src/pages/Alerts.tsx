...
import { ALERT_TYPE_LABELS, ALERT_STATUS_COLORS, ALERT_LEVEL_LABELS, ALERT_STATUS_LABELS } from '@/utils/constants';
...
export default function Alerts() {
  ...
  const handleExportAlerts = () => {
    ...
    const exportData = visibleAlerts.map(alert => {
      const dc = dataCenters.find(d => d.id === alert.dataCenterId);
      const endTime = alert.resolutionTime || Date.now();
      const durationMs = endTime - alert.startTime;
      const durationSec = durationMs / 1000;

      return {
        '机房名称': getDataCenterName(alert.dataCenterId),
        '城市': getCityName(dc?.city || ''),
        '告警级别': ALERT_LEVEL_LABELS[alert.level],
        '状态': ALERT_STATUS_LABELS[alert.status],
        '负责人': getUserName(alert.assignee),
        '触发时间': formatDateTime(alert.startTime),
        '处理耗时': formatDuration(durationSec),
      };
    });

    const worksheet = XLSX.utils.json_to_sheet(exportData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, '告警列表');
    ...
    const fileName = `告警列表_${timestamp}.xlsx`;

    XLSX.writeFile(workbook, fileName);
    message.success(`成功导出 ${exportData.length} 条告警记录`);
  };
